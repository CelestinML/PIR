...
public class Billiev2 : MonoBehaviour
{
    ...
    {
        List<GameObject> obstacles = FindObstacles();

        List<Vector3> position_obstacles = new List<Vector3>();

        foreach (GameObject obstacle in obstacles)
        {
            position_obstacles.Add(obstacle.transform.localPosition);
        ...
        {
            if (obs.y < lowest1 && obs.y > transform.parent.localPosition.y)
            {
                lowest1 = obs.y;
            }
    ...
    private List<GameObject> FindObstacles()
    {
        ...
        {
            GameObject obstacle = environment.GetChild(i).gameObject;
            if (obstacle.tag == "Asteroid" || obstacle.tag == "Undestructible")
            {
