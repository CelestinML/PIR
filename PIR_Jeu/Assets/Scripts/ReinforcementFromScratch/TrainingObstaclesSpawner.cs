...
public class TrainingObstaclesSpawner : MonoBehaviour
{
    ...
    private void SpawnAsteroids()
    {
        ...
        for (int i = 0; i < number_of_asteroids; i++)
        {
            ...
            if (Random.value >= 1.0f / 5.0f)
            {
                ...
                obstacle.GetComponent<TrainingChuteAsteroide>().falling_speed = min_fall_speed + Random.value * (max_fall_speed - min_fall_speed);
                //On détermine la vitesse et le sens de rotation de l'astéroide
                if (Random.value < 0.5)
                    obstacle.GetComponent<TrainingChuteAsteroide>().rotation_speed = min_rotation_speed + Random.value * (max_rotation_speed - min_rotation_speed);
                else
                    obstacle.GetComponent<TrainingChuteAsteroide>().rotation_speed = -(min_rotation_speed + Random.value * (max_rotation_speed - min_rotation_speed));
                //On active le script de déplacement de l'astéroide (après avoir déterminé les paramètres uniquement)
                obstacle.GetComponent<TrainingChuteAsteroide>().enabled = true;
            }
            else
            {
                ...
                obstacle.GetComponent<TrainingDeplacementVaisseau>().falling_speed = min_fall_speed + Random.value * (max_fall_speed - min_fall_speed);
                //On active le script de déplacement du vaisseau (après avoir déterminé les paramètres uniquement)
                obstacle.GetComponent<TrainingDeplacementVaisseau>().enabled = true;
            }
